...
```bash
cd docs-generation
pwsh ./Generate-DocGeneration.Steps.ToolFamilyCleanup.ps1
```
...
```bash
cd docs-generation
pwsh ./GenerateDocGeneration.Steps.ToolFamilyCleanup-multifile.ps1
```
...
2. **Phase 1.5**: `FamilyStructureBuilder` loads deterministic H2 headings, establishes canonical section order, and emits `FamilyStructureContext`
   ...
   - Before stitching, `ParameterCrossCheckService` loads each tool's v2 canonical
     parameter manifest via `CanonicalParameterManifestLoader` and strips hallucinated
     parameter rows not present in the manifest. On `ParameterManifestException` (e.g.,
     legacy format or missing file), the exception propagates — no silent fallback.
...
### Generate-DocGeneration.Steps.ToolFamilyCleanup.ps1 (Single-Phase)

**Purpose**: Process complete tool family files with full cleanup

**Usage:**
```bash
pwsh ./Generate-DocGeneration.Steps.ToolFamilyCleanup.ps1 [options]
```
...
```bash
pwsh ./Generate-DocGeneration.Steps.ToolFamilyCleanup.ps1 -InputDir "./custom/input" -OutputDir "./custom/output"
```

### GenerateDocGeneration.Steps.ToolFamilyCleanup-multifile.ps1 (Multi-Phase)

**Purpose**: Assemble and clean tool families from individual tool files, avoiding token limits

**Usage:**
```bash
pwsh ./GenerateDocGeneration.Steps.ToolFamilyCleanup-multifile.ps1 [options]
```
...
**File**: `DocGeneration.Steps.ToolFamilyCleanup/prompts/tool-family-cleanup-system-prompt.txt`
...
**File**: `DocGeneration.Steps.ToolFamilyCleanup/prompts/tool-family-cleanup-user-prompt.txt`
...
└── DocGeneration.Steps.ToolFamilyCleanup.csproj           # Project file
```
