...
public class AcrolinxComplianceSectionTests
{
    ...
    {
        var path = Path.Combine(ProjectRoot, "mcp-tools", project, folder, file);
    ...
    public void SharedPrompt_Step3_ContainsAcrolinxSection()
    {
        var content = File.ReadAllText(Path.Combine(
            ProjectRoot, "mcp-tools", "DocGeneration.Steps.ToolGeneration.Improvements", "prompts", "system-prompt.txt"));
        var dataDir = Path.Combine(ProjectRoot, "mcp-tools", "data");
    ...
    {
        var content = File.ReadAllText(Path.Combine(
            ProjectRoot, "mcp-tools", "DocGeneration.Steps.ToolFamilyCleanup", "prompts", "tool-family-cleanup-system-prompt.txt"));
        var dataDir = Path.Combine(ProjectRoot, "mcp-tools", "data");
        content = Shared.PromptTokenResolver.Resolve(content, dataDir);
        Assert.Contains("present tense", content, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("contraction", content, StringComparison.OrdinalIgnoreCase);
