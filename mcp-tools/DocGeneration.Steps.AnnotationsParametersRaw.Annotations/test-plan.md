# DocGeneration.Steps.AnnotationsParametersRaw.Annotations Test Plan

This document outlines the test plan for the `DocGeneration.Steps.AnnotationsParametersRaw.Annotations` project. Tests should be placed in a sibling `DocGeneration.Steps.AnnotationsParametersRaw.Annotations.Tests` project using xUnit (consistent with existing test projects in this solution).
...
DocGeneration.Steps.AnnotationsParametersRaw.Annotations is invoked by the pipeline (`start.sh` → Step 1) with **only two flag combinations**:
...
| `GenerateCommonPage_ListsDocGeneration.Core.SharedParams` | Common tools page documents shared parameters from `SourceDiscoveredCommonParams` |
...
- **Framework**: xUnit (consistent with `DocGeneration.Steps.ExamplePrompts.Validation.Tests`, `DocGeneration.Core.Shared.Tests`, `DocGeneration.Core.GenerativeAI.Tests`, `DocGeneration.Core.TextTransformation.Tests`, `DocGeneration.Core.TemplateEngine.Tests`)
