...
DocGeneration.Steps.AnnotationsParametersRaw.Annotations is called during **Step 1** of the generation pipeline (`1-Generate-AnnotationsParametersRaw-One.ps1`). Both annotation and parameter files are generated unconditionally on every invocation — the `--annotations` flag controls only the tool-annotations summary output.
...
```
DocGeneration.Steps.AnnotationsParametersRaw.Annotations template <template-file> <data-file> <output-file>
```
...
External:
└── DocGeneration.Core.TemplateEngine                  Handlebars compilation + custom helpers (shared library)
```
...
|---|---|
| `DocGeneration.Core.TemplateEngine` | Handlebars template rendering and custom helpers |
| `DocGeneration.Core.Shared` | File naming utilities, brand mapping loader, logging |
| `DocGeneration.Core.NaturalLanguage` | Text cleanup and normalization |
...
|---|---|---|
| `CompleteToolGenerator` | `DocGeneration.Steps.ToolGeneration.Composition` | Single-file tool docs |
| `ExamplePromptGenerator` | `DocGeneration.Steps.ExamplePrompts.Generation` | AI example prompts |
| `ToolFamilyPageGenerator` | `DocGeneration.Steps.ToolFamilyCleanup` | Tool family assembly |
...
| `ExamplePromptsResponse` | `DocGeneration.Steps.ExamplePrompts.Generation` | AI response model |
...
dotnet run --project DocGeneration.Steps.AnnotationsParametersRaw.Annotations --configuration Release -- \
