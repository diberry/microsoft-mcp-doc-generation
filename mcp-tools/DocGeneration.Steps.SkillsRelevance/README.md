# DocGeneration.Steps.SkillsRelevance
...
```bash
dotnet run --project DocGeneration.Steps.SkillsRelevance -- <service-name> [options]
```
...
dotnet run --project DocGeneration.Steps.SkillsRelevance -- aks

# Analyze skills for Azure Storage with custom output
dotnet run --project DocGeneration.Steps.SkillsRelevance -- storage --output-path ./generated/skills-relevance

# Analyze with higher minimum relevance threshold
dotnet run --project DocGeneration.Steps.SkillsRelevance -- keyvault --min-score 0.3

# Include all skills (not just relevant ones)
dotnet run --project DocGeneration.Steps.SkillsRelevance -- azure --all-skills
```
...
- `.NET 9.0`
- `DocGeneration.Core.Shared` project (for `LogFileHelper`)
- GitHub REST API (unauthenticated or with `GITHUB_TOKEN`)

## Running Tests

```bash
dotnet test DocGeneration.Steps.SkillsRelevance.Tests/
```
