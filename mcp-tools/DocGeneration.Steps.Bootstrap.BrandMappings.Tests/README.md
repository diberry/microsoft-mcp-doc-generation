# DocGeneration.Steps.Bootstrap.BrandMappings.Tests

Unit tests for the DocGeneration.Steps.Bootstrap.BrandMappings tool.

## Overview

DocGeneration.Steps.Bootstrap.BrandMappings validates that all MCP CLI namespaces have brand mappings. If new namespaces are found, it uses GenAI to suggest brand mapping entries. This test suite verifies the validator's core functionality.
...
The tests focus on top-level inputs and outputs of the DocGeneration.Steps.Bootstrap.BrandMappings command-line application:
...
Since `DocGeneration.Steps.Bootstrap.BrandMappings` is a console application with an internal `Main` method, tests:
...
## DocGeneration.Core.Shared Infrastructure
