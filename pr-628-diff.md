diff --git a/docs/prds/prd-627-npm-to-dotnet-migration.md b/docs/prds/prd-627-npm-to-dotnet-migration.md
new file mode 100644
index 00000000..ac01da64
--- /dev/null
+++ b/docs/prds/prd-627-npm-to-dotnet-migration.md
@@ -0,0 +1,483 @@
+# PRD — Migrate `mcp-cli-metadata` from NPM to .NET
+
+| Field | Value |
+|-------|-------|
+| **Issue** | #627 |
...
+
+`mcp-cli-metadata` implies the folder is a test harness for npm package updates. In practice it is the **pipeline bootstrap data source** — it stores version snapshots, diff files, and CLI examples that feed every Step 0 run. The name misdirects contributors who read it as "test infra, safe to ignore."
+
+---
+
+## 3. Proposed Solution
...
+
+4. **Rename the folder** — `mcp-cli-metadata/` → `mcp-cli-metadata/`. This name describes the actual purpose: storing versioned CLI metadata snapshots and tooling for the pipeline.
+
...
+|---|-----------|-------------|
+| AC1 | Folder `mcp-cli-metadata/` is renamed to `mcp-cli-metadata/` in the repo | `git ls-files mcp-cli-metadata/ \| wc -l` > 0; `git ls-files mcp-cli-metadata/ \| wc -l` = 0 |
...
+| AC5 | `build-and-test.yml` path triggers updated to `mcp-cli-metadata/**` | File content confirms no `mcp-cli-metadata` path trigger |
...
+| AC7 | `test-azure-mcp-update.yml` updated or replaced with .NET equivalent | File references `mcp-cli-metadata/` not `mcp-cli-metadata/` |
+| AC8 | All version snapshot JSON files preserved in `mcp-cli-metadata/` (git history retained) | `git log --oneline mcp-cli-metadata/` shows history from `mcp-cli-metadata/` via `git mv` |
...
+| AC11 | No references to `mcp-cli-metadata` remain in non-historical files | `grep -r "mcp-cli-metadata" --include="*.md" --include="*.yml" --include="*.ps1" --include="*.sh" --include="*.cs" .` returns 0 matches (excluding git log and this PRD) |
...
+
+**Deliverable:** `mcp-cli-metadata/` renamed to `mcp-cli-metadata/` via `git mv`; all references updated.
+
+**Steps:**
+1. `git mv mcp-cli-metadata mcp-cli-metadata` (preserves history).
...
+3. Update `start.sh` if it has any direct `mcp-cli-metadata` references.
+
...
+| Workflow | Change |
+|----------|--------|
+| `build-and-test.yml` | Path trigger `mcp-cli-metadata/**` → `mcp-cli-metadata/**` |
...
+| `generate-docs.yml` | Update any references to `mcp-cli-metadata` |
+
...
+| Resource | Relevance |
+|----------|-----------|
+| `mcp-cli-metadata/package.json` | Primary artifact being replaced |
+| `mcp-cli-metadata/README.md` | Documents current NPM integration |
+| `mcp-cli-metadata/test/*.test.js` | Tests to migrate to xUnit |
...
+| `mcp-tools/DocGeneration.PipelineRunner.Tests/Unit/BootstrapStepTests.cs` | References `mcp-cli-metadata` — update path constants |
...
+
+1. `git ls-files mcp-cli-metadata/` returns **empty** (folder does not exist).
+2. `git ls-files mcp-cli-metadata/` returns **non-empty** (renamed folder exists with snapshot JSON files).
+3. `grep -r "mcp-cli-metadata" --include="*.yml" --include="*.ps1" --include="*.sh" --include="*.cs" --include="*.md" . | grep -v "^./docs/prds/prd-627"` returns **0 matches**.
...
+## 10. Scope — In Scope
+
+- Rename `mcp-cli-metadata/` → `mcp-cli-metadata/` via `git mv`
...
+- Update all documentation files that reference `mcp-cli-metadata`
...
+| Old folder gone (negative) | `git -C . ls-files mcp-cli-metadata/ \| wc -l` | `0` |
+| No npm references in scripts (negative) | `grep -r "npm run\|npm install\|npx azmcp" mcp-tools/scripts/preflight.ps1` | No matches |
+| No old folder refs in workflows (negative) | `grep -r "mcp-cli-metadata" .github/workflows/` | No matches |
...
+| No stray references (negative) | `grep -r "mcp-cli-metadata" --include="*.yml" --include="*.ps1" --include="*.sh" --include="*.cs" . \| grep -v "prd-627"` | No matches |
...
+- **Output targets:**
+  - New folder: `mcp-cli-metadata/` (renamed from `mcp-cli-metadata/`)
...
+
+### In Scope
+- `mcp-cli-metadata/` rename and Node.js removal
...
+- All 12+ documentation files that reference `mcp-cli-metadata`
...
+- **Cleanup Criteria:** PR #627-impl is merged, `mcp-cli-metadata/` no longer exists in the repo, `mcp-cli-metadata/` is the new canonical folder, CI is green, and issue #627 is closed by Dina. No recurring trigger needed after migration — the ongoing nightly workflow is self-contained within `update-azure-mcp.yml`.
+
+---
+
...
+| Q | Answer |
+|---|--------|
+| Q1 — Observable outcome | `mcp-cli-metadata/` no longer exists. `mcp-cli-metadata/` exists with .NET-based extraction tooling. `preflight.ps1` produces `cli-output.json`, `cli-namespace.json`, `cli-version.json` without any npm calls. All CI workflows reference the new folder. |
+| Q2 — Verification | `grep -r "mcp-cli-metadata" --include="*.yml" --include="*.ps1" --include="*.cs" .` → 0 results. `dotnet test mcp-doc-generation.sln` → passes. `git ls-files mcp-cli-metadata/ \| wc -l` → 0. |
