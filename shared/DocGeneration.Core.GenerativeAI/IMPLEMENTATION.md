...
2. **Updated** `DocGeneration.Steps.AnnotationsParametersRaw.Annotations.csproj` to reference `AzureOpenAIClient`
...
- `DocGeneration.Steps.AnnotationsParametersRaw.Annotations` - Succeeded (with new reference)
- `DocGeneration.Core.Shared`, `DocGeneration.Core.NaturalLanguage`, `DocGeneration.Utilities.ToolMetadataExtractor` - Succeeded
...
The `DocGeneration.Steps.AnnotationsParametersRaw.Annotations` project can now use this library to generate example prompts:
