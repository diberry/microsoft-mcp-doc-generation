...
try {
    ...
    # Build the DocGeneration.Steps.ExamplePrompts.Validation project
    Write-Info ""
    Write-Info "Building DocGeneration.Steps.ExamplePrompts.Validation..."
    $scriptDir = $PSScriptRoot
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "DocGeneration.Steps.ExamplePrompts.Validation build failed"
    }
    Write-Success "✓ DocGeneration.Steps.ExamplePrompts.Validation built successfully"
    Write-Info ""
    ...
    if ($exitCode -ne 0 -and $exitCode -ne 1) {
        throw "DocGeneration.Steps.ExamplePrompts.Validation failed with exit code: $exitCode"
    }
