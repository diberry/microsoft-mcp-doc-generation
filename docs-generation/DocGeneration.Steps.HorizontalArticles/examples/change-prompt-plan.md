...
| 3.3 | Update `MergeData()` | `DocGeneration.Steps.HorizontalArticles.cs` | Map new fields in the merge step. |
| 3.4 | Update `RenderAndSaveArticle()` | `DocGeneration.Steps.HorizontalArticles.cs` | Add new fields to the template data dictionary. |
| 3.5 | Update `ValidateAIContent()` | `DocGeneration.Steps.HorizontalArticles.cs` | Adjust validations for new fields (e.g., validate `titleVerb` is a single word, `getStartedPrompts` has exactly 3 items). |
...
| 4.2 | Switch template path | `DocGeneration.Steps.HorizontalArticles.cs` | Update `TEMPLATE_PATH` constant to point to the new template. |
