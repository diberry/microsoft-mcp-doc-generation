...
try {
    ...
    try {
        ...
        $versionOutput = & npm run get:version 2>&1 | Select-Object -First 1 | ForEach-Object { $_.ToString() }
        $cliVersion = "unknown"
        ...
        $jsonOutput = ($output | ForEach-Object { $_.ToString() }) |
            Where-Object { $_ -and -not $_.StartsWith("npm") -and -not $_.StartsWith(">") } |
        ...
        # Extract JSON from output (filter out npm warnings/notices)
        $jsonNamespaceOutput = ($output | ForEach-Object { $_.ToString() }) |
            Where-Object { $_ -and -not $_.StartsWith("npm") -and -not $_.StartsWith(">") } |
            Join-String -Separator "`n"
