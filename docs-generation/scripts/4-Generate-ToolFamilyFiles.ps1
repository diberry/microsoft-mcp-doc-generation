...
try {
    ...
    if ($skipRaw) {
        & "$scriptDir\2-Generate-ToolGenerationAndAIImprovements.ps1" -OutputPath $OutputPath -SkipRaw -MaxTokens $MaxTokens
    } else {
        & "$scriptDir\2-Generate-ToolGenerationAndAIImprovements.ps1" -OutputPath $OutputPath -MaxTokens $MaxTokens
    }
    if ($LASTEXITCODE -ne 0) {
