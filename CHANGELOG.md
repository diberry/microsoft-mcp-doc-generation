...
- **CLI step validation now accepts explicit Bootstrap requests** — `PipelineRequest.Validate()` previously used `DefaultSteps` (`1-6`) as both the default run set and the fallback validation allowlist. That caused `--steps 0,1` to fail during CLI parse-time validation even though Bootstrap (`step 0`) is a real registered step and deterministic regression CI correctly resolves data-file changes to it. Fix: added `AllValidSteps` (`0-6`) for validation while keeping `DefaultSteps` (`1-6`) unchanged so Bootstrap remains opt-in. Added regression coverage for `--steps 0,1`. Closes #615.
