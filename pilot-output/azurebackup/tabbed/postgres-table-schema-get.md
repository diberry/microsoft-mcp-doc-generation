...
This tool executes `postgres table schema get` via MCP Server.
...
```azurecli
azmcp postgres table schema get --resource-group <resource-group> --user <user> --server <server> --database <database> --auth-type <auth-type> --password <password> --table <table>
```
...
| `--table` | string | - | The PostgreSQL table to be accessed. |
