...
This tool executes `sql db rename` via MCP Server.
...
```azurecli
azmcp sql db rename --resource-group <resource-group> --server <server> --database <database> --new-database-name <new-database-name>
```
...
| `--new-database-name` | string | The new name for the Azure SQL Database. |
