...
This tool executes `eventgrid events publish` via MCP Server.
...
```azurecli
azmcp eventgrid events publish --resource-group <resource-group> --topic <topic> --data <data> --schema <schema>
```
...
| `--data` | string | - | The event data as JSON string to publish to the Event Grid topic. |
| `--schema` | string | - | The event schema type (CloudEvents, EventGrid, or Custom). Defaults to EventGrid. |
